 ...
 */
void search_combinations_recursive( int target, unsigned int weight, unsigned int weight_id, unsigned int *nomenclature,
                                    unsigned int *weights_ids, unsigned int *nomenclature_size, unsigned int *current_combination, int *weights_counter,
                                    unsigned int *weights_ids_sums, int *number_of_combinations, unsigned int **combinations,
                                    unsigned int *max_combinations, const unsigned int *original_nomenclature_size )
{
    ( *weights_counter )++;
    ...
    if ( target == 0 )
    {
        ...
        weights_ids_sums[*number_of_combinations] =
            weight_id;  // если такой комбинации не было, добавляем сумму её id-шников
    ...
    unsigned int *sub_nomenclature_size =
        &sub_nomenclature_size_value;     // размер новой номенклатуры
    ...
    for ( unsigned int weight_index = 0; weight_index < *nomenclature_size; weight_index++ )
    {
        ...
        unsigned int current_weight =
            nomenclature[weight_index];                   // забираем гирю из номенклатуры
        ...
        {
            if ( weight_index ==
                    weight_index_2 ) // гирю current_weight нужно пропустить, т.к. в новой номенклатуре её быть не должно
                continue;
        ...
        // погружаемся ниже
        search_combinations_recursive( target, current_weight, current_weight_id, sub_nomenclature, sub_weights_ids,
                                       sub_nomenclature_size, current_combination, weights_counter, weights_ids_sums, number_of_combinations, combinations,
                                       max_combinations, original_nomenclature_size );
 ...
 */
int solution_recursive( unsigned int *nomenclature, unsigned int *nomenclature_size, unsigned int *target,
                        unsigned int ***out_combinations )
{
    ...
    unsigned int *weights_current_combination = ( unsigned int * )calloc( *nomenclature_size,
                                                                          sizeof( unsigned int ) ); // массив с 'кандидатом' в готовую комбинацию
    ...
    *out_combinations = ( unsigned int ** )calloc( *max_combinations,
                                                   sizeof( unsigned int * ) ); // выделяем место для комбинаций
    ...
    for ( unsigned int weight_index = 0; weight_index < *nomenclature_size; weight_index++ )
    {
        ...
        ( *weight_counter ) =
            -1; // на каждой итерации необходимо сбрасывать счётчик до -1, т.к. здесь вход в рекурсию
        ...
        search_combinations_recursive( *target, current_weight, current_weight_id, sub_nomenclature, sub_weights_ids,
                                       sub_nomenclature_size, weights_current_combination, weight_counter, weights_ids_sums, number_of_combinations,
                                       *out_combinations, max_combinations, nomenclature_size );
 ...
 * @return int Код ошибки, вернёт 0, если нет ошибки
 */
int push_combination( unsigned int ***combinations, unsigned int *combinations_size, unsigned int *curr_combination,
                      unsigned int *original_nomenclature_size )
{
    ...
    for ( unsigned int **combination = *combinations, **combination_end = *combinations + *combinations_size;
 ...
 * @return int Размер массива out_combinations (по совместительству - количество комбинаций для данного целевого веса)
 */
int solution_bits( unsigned int *nomenclature, unsigned int *nomenclature_size, unsigned int *target,
                   unsigned int ***out_combinations )
{
