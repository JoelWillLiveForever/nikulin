...
int main()
{
    ...
    int hash_sums[] = {55, 23, 421, 90, 10, 4, -8, 92, 67, 111};    // хэш коды каждой гири, соответствие по индексу
    ...
    for (int sub_index = 0; sub_index < weights_amount; sub_index++)
    {
        ...
        for (int j = 0; j < weights_amount; j++)
        {
            ...
            sub_hashes_remains[remains_curr_index] = hash_sums[j];
            remains_curr_index++;
        }

        int sub = weights[sub_index];           // выбранная гиря
        int sub_hash = hash_sums[sub_index];    // хэш код выбранной гири
...
//    for (int i = 0; i < SAVED_HASHSUMS_SIZE; i++) 
  //  {
//        printf("\nsaved_hashsums[%d] = %6d\troots: ", i, saved_hashsums[i]);
...
//                {
  //                  if (j1 == j3) continue;
    //                if (hash_sum == saved_hashsums[i]) 
      //              {
        //                for (int k = 0; k < 10; k++)
          //                  printf("%d", buffer[k]);
            //            break;
              //      }
                //    hash_sum += hash_sums[j3];                                             
//                    buffer[j3] = weights[j3];
  //              }
    //        }
//        }
  //  }
